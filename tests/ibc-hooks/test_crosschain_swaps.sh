...
export VALIDATOR=$(symphonyd keys show validator -a)
...
chainA tx ibc-transfer transfer transfer $CHANNEL_ID "$VALIDATOR" 600000000note --from validator "${TX_FLAGS[@]}"
...
{
        "weights": "1${DENOM},1note",
        "initial-deposit": "1000000${DENOM},1000000note",
        "swap-fee": "0.01",
        "exit-fee": "0.01",
...
chainB tx wasm store ./bytecode/crosschain_swaps.wasm --from validator --gas auto --gas-prices 0.1note --gas-adjustment 1.3 -y -b block
...
MSG=$(jenv -c '{"swap_contract": $SWAPROUTER_ADDRESS, "channels": [["melody", $CHANNEL_ID]]}')
...
MEMO=$(jenv -c '{"wasm": {"contract": $CROSSCHAIN_SWAPS_ADDRESS, "msg": {"symphony_swap":{"swap_amount":"100","output_denom":"note","slippage":{"twap": {"slippage_percentage":"20", "window_seconds": 10}},"receiver":$VALIDATOR, "on_failed_delivery": "do_nothing"}}}}')
chainA tx ibc-transfer transfer transfer $CHANNEL_ID $CROSSCHAIN_SWAPS_ADDRESS 100note \
