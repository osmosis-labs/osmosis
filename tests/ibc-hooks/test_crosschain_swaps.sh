...
chainA tx ibc-transfer transfer transfer $CHANNEL_ID "$VALIDATOR" 60000000uosmo --from validator "${TX_FLAGS[@]}"
...
MSG=$(jenv -c '{"set_route":{"input_denom":$DENOM,"output_denom":"uosmo","pool_route":[{"pool_id":$POOL_ID,"token_out_denom":"$DENOM"}]}}')
...
MEMO=$(jenv -c '{"wasm": {"contract": $CROSSCHAIN_SWAPS_ADDRESS, "msg": {"osmosis_swap":{"input_coin":{"denom":$DENOM,"amount":"100"},"output_denom":"uosmo","slippage":{"twap": {"slippage_percentage":"20"}},"receiver":$VALIDATOR}}}}')
