...
func (n *Network) GetValidatorRPCClient(validatorIndex int) *rpchttp.HTTP {
	return n.validatorRPC[validatorIndex]
}
...
func (n *Network) WaitUntilHeight(height int64) error {
	...
	doneCondition := func(syncInfo coretypes.SyncInfo) bool {
		...
		if curHeight < height {
			n.t.Logf("current block height is %d, waiting to reach: %d", curHeight, height)
		}

		return syncInfo.CatchingUp || curHeight > height
	}

	for i := range n.GetChain().Validators {
...
func (n *Network) RunValidator(validatorIndex int) (*dockertest.Resource, error) {
	...
	n.t.Logf("starting validator %s; repository: %s; tag %s", runOpts.Name, runOpts.Repository, runOpts.Tag)
...
func (n *Network) getValidatorOptions(valIndex int) *dockertest.RunOptions {
	validator := n.chain.Validators[valIndex]
	return &dockertest.RunOptions{
		Name:      validator.Name,
		NetworkID: n.dockerResources.Network.Network.ID,
		Mounts: []string{
			fmt.Sprintf("%s/:/osmosis/.osmosisd", validator.ConfigDir),
			fmt.Sprintf("%s/scripts:/osmosis", n.workingDirectory),
		},
		Repository: n.dockerImages.OsmosisRepository,
		Tag:        n.dockerImages.OsmosisTag,
		Cmd: []string{
			"start",
		},
	}
