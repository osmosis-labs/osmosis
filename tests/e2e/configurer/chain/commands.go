...
func (n *NodeConfig) CreateConcentratedPosition(from, lowerTick, upperTick string, token0, token1 string, token0MinAmt, token1MinAmt int64, poolId uint64) uint64 {
	...
	// TODO: currently setting gas to "auto" but we need to bound gas limit, issue: https://github.com/osmosis-labs/osmosis/issues/4947
	cmd := []string{"osmosisd", "tx", "concentratedliquidity", "create-position", lowerTick, upperTick, token0, token1, fmt.Sprintf("%d", token0MinAmt), fmt.Sprintf("%d", token1MinAmt), fmt.Sprintf("--from=%s", from), fmt.Sprintf("--pool-id=%d", poolId), "-o json", "--gas=auto", "--gas-prices=0.1uosmo", "--gas-adjustment=1.3"}
