...
func (n *NodeConfig) CreateConcentratedPosition(from, lowerTick, upperTick string, token0Amount, token1Amount string, token0MinAmt, token1MinAmt int64, poolId uint64) uint64 {
	...
	cmd := []string{"osmosisd", "tx", "concentratedliquidity", "create-position", fmt.Sprint(poolId), lowerTick, upperTick, token0Amount, token1Amount, fmt.Sprintf("%d", token0MinAmt), fmt.Sprintf("%d", token1MinAmt), fmt.Sprintf("--from=%s", from), "--gas=500000", "--fees=1250uosmo", "-o json"}
