...
var f = InitIntegrationFixture(&testing.T{})
...
func TestConcentratedLiquidity(t *testing.T) {
	...
	node, err := chainA.GetAvailableNode()
	...
	assert.DeepEqual(t, concentratedPool.GetId(), poolID)
	assert.DeepEqual(t, concentratedPool.GetToken0(), denom0)
	assert.DeepEqual(t, concentratedPool.GetToken1(), denom1)
	assert.DeepEqual(t, concentratedPool.GetTickSpacing(), tickSpacing)
	assert.DeepEqual(t, concentratedPool.GetPrecisionFactorAtPriceOne(), sdk.NewInt(precisionFactorAtPriceOne))
	assert.DeepEqual(t, concentratedPool.GetSwapFee(sdk.Context{}), sdk.MustNewDecFromStr(swapFee))
	...
	assert.DeepEqual(t, len(positionsAddress1), 2)
	assert.DeepEqual(t, len(positionsAddress2), 1)
	assert.DeepEqual(t, len(positionsAddress3), 2)
	...
	validateCLPosition := func(position types.FullPositionByOwnerResult, poolId uint64, lowerTick, upperTick int64) {
		assert.DeepEqual(t, position.PoolId, poolId)
		assert.DeepEqual(t, position.LowerTick, int64(lowerTick))
		assert.DeepEqual(t, position.UpperTick, int64(upperTick))
	}
...
func TestGeometricTwapMigration(t *testing.T) {
	...
	balances, err := node.QueryBalances(chainA.NodeConfigs[0].PublicAddress)
	fmt.Println("balances", balances, err)
...
// 	chainANode, err := chainA.GetAvailableNode()
// 	assert.NilError(t, err)
...
// 	assert.NilError(t, err)
// 	assert.DeepEqual(t, len(contracts), 1, "Wrong number of contracts for the rate limiter")
...
// 	assert.NilError(t, err)
// 	assert.DeepEqual(t, len(contracts), 1, "Wrong number of contracts for the counter")
