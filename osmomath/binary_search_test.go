...
func init() {
	negCubicFConstant = NewBigDec(1 << 62).Power(3).Neg()
}
...
func TestBinarySearchBigDec(t *testing.T) {
	...
	tests := map[string]binarySearchTestCase{
		"cubic f, no err tolerance, converges":     {cubicF, bigZero, twoTo50, twoTo25PlusOneCubed, withinOne, 51, twoTo25PlusOne, false},
		"cubic f, no err tolerance, not converges": {cubicF, bigZero, twoTo50, twoTo25PlusOneCubed, withinOne, 10, twoTo25PlusOne, true},
		...
		"neg cubic f, no err tolerance, converges": {negCubicF, bigZero, twoTo50,
...
func TestBinarySearchRoundingBehavior(t *testing.T) {
	...
	tests := map[string]binarySearchTestCase{
		"lineF, roundup within 2^30, target 2^32 + 2^30 + 1, expected=2^32 + 2^31": {f: lineF,
			lowerbound: bigZero, upperbound: twoTo50,
