...
func (s *decimalTestSuite) TestPowerInteger() {
	...
	tests := map[string]struct {
		...
		"geom twap overflow: 2^log_2{max spot price + 1}": {
			...
			exponent: uint64(osmomath.BigDecFromSDKDec(osmomath.MaxSpotPrice).Add(osmomath.OneBigDec()).LogBase2().TruncateInt().Uint64()),
		...
		"geom twap overflow: 2^log_2{max spot price / 2 - 2017}": { // 2017 is prime.
			base:     osmomath.TwoBigDec,
			exponent: uint64(osmomath.BigDecFromSDKDec(osmomath.MaxSpotPrice.Quo(osmomath.NewDec(2)).Sub(osmomath.NewDec(2017))).LogBase2().TruncateInt().Uint64()),
