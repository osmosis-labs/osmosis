...
func (s *decimalTestSuite) TestLog2() {
	...
	tests := map[string]struct {
		...
		"log_2{Max Spot Price} = 128": {
			initialValue: osmomath.BigDecFromSDKDec(MaxSpotPrice), // 2^128 - 1
...
func (s *decimalTestSuite) TestPowerInteger() {
	...
	tests := map[string]struct {
		...
		"geom twap overflow: 2^log_2{max spot price + 1}": {
			...
			exponent: uint64(osmomath.BigDecFromSDKDec(MaxSpotPrice).Add(osmomath.OneDec()).LogBase2().TruncateInt().Uint64()),
		...
		"geom twap overflow: 2^log_2{max spot price}": {
			base:     osmomath.TwoBigDec,
			exponent: uint64(osmomath.BigDecFromSDKDec(MaxSpotPrice).LogBase2().TruncateInt().Uint64()),
		...
		"geom twap overflow: 2^log_2{max spot price / 2 - 2017}": { // 2017 is prime.
			base:     osmomath.TwoBigDec,
			exponent: uint64(osmomath.BigDecFromSDKDec(MaxSpotPrice.Quo(sdk.NewDec(2)).Sub(sdk.NewDec(2017))).LogBase2().TruncateInt().Uint64()),
