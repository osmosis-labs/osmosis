	...
	}{
		{
			...
			func() {
				...
				for i := uint64(1); i < v16.DaiOsmoPoolId; i++ {
					s.PrepareBalancerPoolWithCoins(sdk.NewCoin("note", osmomath.NewInt(10000000000)), sdk.NewCoin("ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", defaultDaiAmount))
				}

				// Create DAI / OSMO pool
				s.PrepareBalancerPoolWithCoins(sdk.NewCoin("ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7", defaultDaiAmount), sdk.NewCoin("note", osmomath.NewInt(10000000000)))
			},
			func() {
				...
				s.Require().Equal(communityPoolBalancePre.AmountOf("note").Sub(respectiveOsmo.Amount).String(), communityPoolBalancePost.AmountOf("note").String())
				...
				s.Require().Equal(communityPoolBalancePost.AmountOf("note").String(), feePoolCommunityPoolPost.AmountOf("note").TruncateInt().String())
...
func upgradeProtorevSetup(s *UpgradeTestSuite) error {
	...
	devFee := sdk.NewCoin("note", osmomath.NewInt(1000000))
	if err := s.App.ProtoRevKeeper.SetDeveloperFees(s.Ctx, devFee); err != nil {
		return err
	}

	fundCoin := sdk.NewCoins(sdk.NewCoin("note", osmomath.NewInt(1000000)))
...
func verifyProtorevUpdateSuccess(s *UpgradeTestSuite) {
	...
	s.Require().NoError(err)
	s.Require().Equal(s.App.BankKeeper.GetBalance(s.Ctx, devAcc, "note"), sdk.NewCoin("note", osmomath.NewInt(1000000)))
