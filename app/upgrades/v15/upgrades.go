...
func CreateUpgradeHandler(
	...
	return func(ctx sdk.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
		...
		setRateLimits(ctx, keepers.AccountKeeper, keepers.WasmKeeper)
...
func setRateLimits(ctx sdk.Context, accountKeeper *authkeeper.AccountKeeper, wasmKeeper *wasmkeeper.Keeper) {
	govModule := accountKeeper.GetModuleAddress(govtypes.ModuleName)
	contractKeeper := wasmkeeper.NewGovPermissionKeeper(wasmKeeper)

	paths := []string{
		`{"channel_id": "any", "denom": "ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/D189335C6E4A68B513C10AB227BF1C1D38C746766278BA3EEB4FB14124F1D858",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/D1542AA8762DB13087D8364F3EA6509FD6F009A34F00426AF9E4F9FA85CBBF1F",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/EA1D43981D5C9A1C4AAEA9C23BB1D4FA126BA9BC7020A25E0AE4AA841EA25DC5",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/6AE98883D4D5D5FF9E50D7130F1305DA2FFA0C652D1DD9C123657C6B4EB2DF8A",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/987C17B11ABC2B20019178ACE62929FE9840202CE79498E29FE8E5CB02B7C0A4",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/0CD3A0285E1341859B5E86B6AB7682F023D03E97607CCC1DC95706411D866DF7",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/46B44899322F3CD854D2D46DEEF881958467CDD4B3B10086DA49296BBED94BED",
          "quotas":
            [
		...
		`{"channel_id": "any", "denom": "ibc/E6931F78057F7CC5DA0FD6CEF82FF39373A6E0452BF1FD76910B93292CF356C1",
          "quotas":
            [
	...
	rateLimitingContract, err := sdk.AccAddressFromBech32("osmo17r7qdw2zk6jyw62cvwm6flmhtj9q7zd26r8zc6sqyf0pnaq46cfss8hgxg")
	if err != nil {
		panic("contract address improperly formatted")
	}
	for _, denom := range paths {
		res, err := contractKeeper.Execute(ctx, rateLimitingContract, govModule, []byte(denom), nil)
		fmt.Println(string(res)) // printing for potential debugging purposes
		if err != nil {
			panic(err)
		}
	}
