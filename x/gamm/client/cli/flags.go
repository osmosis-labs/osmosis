...
func FlagSetJoinPool() *flag.FlagSet {
	...
	fs.StringArray(FlagMaxAmountsIn, []string{""}, "Maximum amount of each denom to send into the pool (specify multiple denoms with: --max-amounts-in=1udym --max-amounts-in=1uion)")
	return fs
}
