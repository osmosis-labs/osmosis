...
func NewReplaceMigrationRecordsProposal(title, description string, records []GammToConcentratedPoolLink) govtypes.Content {
...
func (p ReplaceMigrationRecordsProposal) String() string {
	...
	for _, record := range p.Records {
		recordsStr = recordsStr + fmt.Sprintf("(GammPoolID: %d, ClPoolID: %d) ", record.GammPoolId, record.ClPoolId)
	}
...
// NewReplacePoolIncentivesProposal returns a new instance of a replace migration record's proposal struct.
func NewUpdatePoolIncentivesProposal(title, description string, records []GammToConcentratedPoolLink) govtypes.Content {
