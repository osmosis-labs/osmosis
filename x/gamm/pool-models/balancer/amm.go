...
func calcPoolSharesOutGivenSingleAssetIn(
	...
	if tokenAmountIn.Equal(osmomath.NewDec(100000)) {
		fmt.Println("calcPoolSharesOutGivenSingleAssetIn")
		fmt.Println("tokenBalanceIn", tokenBalanceIn)
		fmt.Println("normalizedTokenWeightIn", normalizedTokenWeightIn)
		fmt.Println("poolShares", poolShares)
		fmt.Println("tokenAmountIn", tokenAmountIn)
		fmt.Println("spreadFactor", spreadFactor)
	}
	...
	if tokenAmountIn.Equal(osmomath.NewDec(100000)) {
		fmt.Println("poolAmountOutDec", poolAmountOut)
		fmt.Println("poolAmountOutInt", poolAmountOut.TruncateInt())
	}
