...
func (s *KeeperTestSuite) TestBalancerSpotPriceBounds() {
	...
	tests := []struct {
		...
		},
		{
			...
			quoteDenomPoolInput: sdk.NewCoin(baseDenom, sdk.OneInt()),
			quoteDenomWeight:    osmomath.NewInt(100),
			baseDenomPoolInput:  sdk.NewCoin(quoteDenom, sdk.NewDec(10).PowerMut(19).TruncateInt().Sub(osmomath.NewInt(2))),
			baseDenomWeight:     osmomath.NewInt(100),
			expectError:         true,
		},
		{
			...
			baseDenomWeight:     osmomath.NewInt(100),
			expectedOutput:      osmomath.OneDec().Quo(sdk.NewDec(10).PowerMut(18)),
		},
	}
