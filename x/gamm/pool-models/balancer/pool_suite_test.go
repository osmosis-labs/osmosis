...
func (suite *KeeperTestSuite) TestCalcJoinPoolShares() {
	...
	for _, tc := range testCases {
		...
		suite.T().Run(tc.name, func(t *testing.T) {
			...
			// system under test
			sut := func() {
				shares, liquidity, err := pool.CalcJoinPoolShares(suite.Ctx, tc.tokensIn, tc.spreadFactor)
				if tc.expErr != nil {
					require.Error(t, err)
	...
	for _, tc := range testCases {
		...
		suite.T().Run(tc.name, func(t *testing.T) {
			...
			sut := func() {
				...
				shares, err := pool.JoinPool(suite.Ctx, tc.tokensIn, tc.spreadFactor)
...
func (suite *KeeperTestSuite) TestJoinPoolNoSwap() {
	...
	for _, tc := range testCases {
		...
		suite.T().Run(tc.name, func(t *testing.T) {
			...
			sut := func() {
				preJoinAssets := pool.GetTotalPoolLiquidity(suite.Ctx)
				shares, err := pool.JoinPoolNoSwap(suite.Ctx, tc.tokensIn, tc.spreadFactor)
				postJoinAssets := pool.GetTotalPoolLiquidity(suite.Ctx)
