...
func (msg MsgCreateBalancerPool) ValidateBasic() error {
	...
	err = balancer.ValidateUserSpecifiedPoolAssets(msg.PoolAssets)
	if err != nil {
		return err
	}
...
func (msg MsgCreateBalancerPool) CreatePool(ctx sdk.Context, poolID uint64) (swaproutertypes.PoolI, error) {
	poolI, err := balancer.NewBalancerPool(poolID, *msg.PoolParams, msg.PoolAssets, msg.FuturePoolGovernor, ctx.BlockTime())
	return &poolI, err
}
