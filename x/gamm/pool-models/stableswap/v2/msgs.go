...
func (msg MsgCreateStableswapPool) ValidateBasic() error {
	...
	if len(scalingFactors) != 0 {
		if err = stableswap.ValidateScalingFactors(scalingFactors, len(msg.InitialPoolLiquidity)); err != nil {
			return err
		}
	} else {
	...
	if err = stableswap.ValidatePoolLiquidity(msg.InitialPoolLiquidity, scalingFactors); err != nil {
		return err
	}

	// validation for scaling factor owner
	if err = stableswap.ValidateScalingFactorController(msg.ScalingFactorController); err != nil {
		return err
	}
...
func (msg MsgCreateStableswapPool) CreatePool(ctx sdk.Context, poolId uint64) (swaproutertypes.PoolI, error) {
	stableswapPool, err := stableswap.NewStableswapPool(poolId, *msg.PoolParams, msg.InitialPoolLiquidity,
