	...
	for name, tc := range testcases {
		for _, swapFee := range swapFeeCases {
			suite.Run(getTestCaseName(name, tc, swapFee), func() {
				...
				helpers.TestCalculateAmountOutAndInInverseRelationship(suite.T(), ctx, pool, tc.denomIn, tc.denomOut, tc.initialCalcOut, swapFeeDec, errTolerance)
			})
		}
	}
}
...
func TestCalcSingleAssetJoinShares(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			tokenOutAmount, err := common.SwapAllCoinsToSingleAsset(&p, ctx, exitTokens, tc.tokenIn.Denom, sdk.ZeroDec())
