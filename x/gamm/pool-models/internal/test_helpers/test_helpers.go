...
func TestCalculateAmountOutAndIn_InverseRelationship(
	...
	// If the pool is extremely imbalanced, we expect there to be drastically
	// amplified error that will fall outside our usual bounds. For these cases,
	// especially for stableswap, all we really care about is that they are safe
	// and were rounded correctly.
