...
import (
	...
	clt "github.com/osmosis-labs/osmosis/v24/x/concentrated-liquidity/types"
)
...
func (s *KeeperTestSuite) TestBeginBlockLazyMigration() {
	...
	clt.FinalIncentiveAccumulatorPoolIDsToMigrate = map[uint64]struct{}{}
	clt.FinalIncentiveAccumulatorPoolIDsToMigrate[nextPoolId-1] = struct{}{}
	...
	_, err = s.App.ConcentratedLiquidityKeeper.CreateIncentive(
		...
		s.TestAccs[0],
		totalIncentiveAmount,
		emissionRatePerSecDec, s.Ctx.BlockTime(), types.DefaultAuthorizedUptimes[0],
	)
	s.Require().NoError(err)
