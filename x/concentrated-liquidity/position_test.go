...
func (s *KeeperTestSuite) TestValidateAndFungifyChargedPositions() {
	var (
		...
		blockTimeIncreaseOneDay = time.Hour * 24
	)
	...
	tests := []struct {
		...
		// TODO: add test where the max supported uptime is not authorized
		// only 1ms uptime is authorized. The time that passes is
		// greater than 1 ms but smaller than max supported uptime.
		// Expect: pass because we should be grabbing from params
		// Actual: fail because we currently grab from supported uptimes.
	...
	for _, test := range tests {
		s.Run(test.name, func() {
			...
			lockDuration := cl.FullyChargedDuration + blockTimeIncreaseOneDay
			lockDuration = lockDuration + test.unlockBeforeBlockTimeMs
			...
			s.Ctx = s.Ctx.WithBlockTime(s.Ctx.BlockTime().Add(blockTimeIncreaseOneDay))
