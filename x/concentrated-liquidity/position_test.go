...
func (s *KeeperTestSuite) TestNegativeTickRange_SpreadFactor() {
	...
	estimateCoinOneIn := func(toTick int64) sdk.Coin {
		...
		amountOneIn := math.CalcAmount1Delta(osmomath.BigDecFromDec(pool.GetLiquidity()), pool.GetCurrentSqrtPrice(), s.tickToSqrtPrice(toTick), true)
...
// 			initialUserPositions, err := s.App.ConcentratedLiquidityKeeper.GetUserPositions(s.Ctx, s.TestAccs[0], 1)
// 			s.Require().NoError(err)

// 			newOwnerBalancePre := s.App.BankKeeper.GetAllBalances(s.Ctx, s.TestAccs[1])

// 			// System under test
// 			newPositionIds, err := s.App.ConcentratedLiquidityKeeper.TransferPositions(s.Ctx, tc.positionIds, s.TestAccs[0], s.TestAccs[1])
...
// 				s.Require().Equal(lastPositionId, len(newPositionIds))
// 				for i, newPositionId := range newPositionIds {
// 					newPosition, err := s.App.ConcentratedLiquidityKeeper.GetPosition(s.Ctx, newPositionId)
// 					s.Require().NoError(err)

// 					// All the following should be the same in the new position as it was in the old one.
// 					s.Require().Equal(initialUserPositions[i].UpperTick, newPosition.UpperTick)
// 					s.Require().Equal(initialUserPositions[i].LowerTick, newPosition.LowerTick)
// 					s.Require().Equal(initialUserPositions[i].PoolId, newPosition.PoolId)

// 					// The new position should have the new owner
// 					s.Require().Equal(s.TestAccs[1].String(), newPosition.Address)

// 					// NOTE: Liquidity values are not the same between positions,
// 					newOwnerBalancePost := s.App.BankKeeper.GetAllBalances(s.Ctx, s.TestAccs[1])
// 					fmt.Println("newOwnerBalancePost", newOwnerBalancePost)
// 					fmt.Println("newOwnerBalancePre", newOwnerBalancePre)
// 				}
// 			}
// 		})
// 	}
...
func (s *KeeperTestSuite) TestTransferPositions() {
	...
	for name, tc := range testcases {
		s.Run(name, func() {
			...
			// System under test
			err = s.App.ConcentratedLiquidityKeeper.TransferPositions(s.Ctx, tc.positionIds, s.TestAccs[0], s.TestAccs[1])
			...
			if tc.expectedError != nil {
				...
				for i := 0; i < lastPositionId; i++ {
					newPosition, err := s.App.ConcentratedLiquidityKeeper.GetPosition(s.Ctx, uint64(i+1))
					s.Require().NoError(err)
					...
					s.Require().Equal(initialUserPositions[i].LowerTick, newPosition.LowerTick)
					s.Require().Equal(initialUserPositions[i].PoolId, newPosition.PoolId)
					s.Require().Equal(initialUserPositions[i].JoinTime, newPosition.JoinTime)
					s.Require().Equal(initialUserPositions[i].Liquidity, newPosition.Liquidity)
