...
var (
	...
	swapOutGivenInCases = map[string]SwapTest{
		...
		"single position within one tick, trade completes but slippage protection interrupts trade early: eth -> usdc": {
			...
			// params
			// liquidity: 		 1517882343.751510418088349649
			// sqrtPriceNext:    70.668238976219012613 which is 4993.999999999999999925: https://www.wolframalpha.com/input?i=70.668238976219012613*70.668238976219012613
			// sqrtPriceCurrent: 70.710678118654752440 which is 5000
			...
			expectedTick: func() int64 {
				tick, _ := math.PriceToTickRoundDownSpacing(sdk.MustNewDecFromStr("4993.999999999999999925"), DefaultTickSpacing)
				return tick
			}(),
	...
	swapOutGivenInSpreadRewardCases = map[string]SwapTest{
		...
		"spread factor 7: single position within one tick, trade completes but slippage protection interrupts trade early: eth -> usdc (1% spread factor)": {
			...
			expectedTick: func() int64 {
				tick, _ := math.PriceToTickRoundDownSpacing(sdk.NewDec(4994), DefaultTickSpacing)
				return tick
			}(),
...
func (s *KeeperTestSuite) getExpectedLiquidity(test SwapTest, pool types.ConcentratedPoolExtension) sdk.Dec {
	...
	newLowerTick, err := math.PriceToTickRoundDownSpacing(test.newLowerPrice, pool.GetTickSpacing())
	s.Require().NoError(err)
	newUpperTick, err := math.PriceToTickRoundDownSpacing(test.newUpperPrice, pool.GetTickSpacing())
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) setupSecondPosition(test SwapTest, pool types.ConcentratedPoolExtension) {
	if !test.secondPositionLowerPrice.IsNil() {
		newLowerTick, err := math.PriceToTickRoundDownSpacing(test.secondPositionLowerPrice, pool.GetTickSpacing())
		s.Require().NoError(err)
		newUpperTick, err := math.PriceToTickRoundDownSpacing(test.secondPositionUpperPrice, pool.GetTickSpacing())
		s.Require().NoError(err)
