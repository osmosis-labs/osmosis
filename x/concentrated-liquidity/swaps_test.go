...
// func (s *KeeperTestSuite) TestOverConsume() {
// 	s.SetupTest()
// 	pool := s.PrepareCustomConcentratedPool(s.TestAccs[0], ETH, USDC, 1, sdk.ZeroDec())

// 	desiredSqrtPriceCurrent := osmomath.MustNewDecFromStr("0.000001000049998750")

// 	desiredPrice := desiredSqrtPriceCurrent.Mul(desiredSqrtPriceCurrent)

// 	amountZeroIn := sdk.NewDec(10).PowerMut(18)
// 	amountOneIn := amountZeroIn.Mul(desiredPrice.SDKDec())

// 	desiredTick, err := math.CalculateSqrtPriceToTick(desiredSqrtPriceCurrent.SDKDec())
// 	s.Require().NoError(err)
...
// 	coinsIn := sdk.NewCoins(sdk.NewCoin(ETH, amountZeroIn.TruncateInt()), sdk.NewCoin(USDC, amountOneIn.TruncateInt()))
// 	s.FundAcc(positionOwner, coinsIn)

// 	// Create full range position with given coins to initalize desired sqrt price
// 	_, _, _, _, err = s.clk.CreateFullRangePosition(s.Ctx, pool.GetId(), positionOwner, coinsIn)
// 	s.Require().NoError(err)

// 	pool, err = s.clk.GetPoolById(s.Ctx, pool.GetId())
// 	s.Require().NoError(err)

// 	s.Require().Equal(desiredSqrtPriceCurrent.SDKDec(), pool.GetCurrentSqrtPrice())

// 	amountIn, _, _ := s.computeSwapAmounts(pool.GetId(), pool.GetCurrentSqrtPrice(), desiredTick, false, false)

// 	s.FundAcc(positionOwner, sdk.NewCoins(sdk.NewCoin(USDC, amountIn.TruncateInt())))
// 	_, err = s.clk.SwapExactAmountIn(s.Ctx, positionOwner, pool, sdk.NewCoin(USDC, amountIn.TruncateInt()), ETH, sdk.ZeroInt(), sdk.ZeroDec())
// 	s.Require().NoError(err)
...
// 	// _, _, _, _, _, _, err = s.clk.CreatePosition(s.Ctx, pool.GetId(), positionOwner, coinsIn, sdk.ZeroInt(), sdk.ZeroInt(), tick-1, tick+1)
// 	// s.Require().NoError(err)
