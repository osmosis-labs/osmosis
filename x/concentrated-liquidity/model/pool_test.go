...
func (suite *ConcentratedPoolTestSuite) TestCalcActualAmounts() {
	var (
		...
		lowerTick            = int64(-99)
		lowerSqrtPriceBigDec = osmomath.BigDecFromDec(tickToSqrtPrice(lowerTick))

		midtick            = int64(2)
		midSqrtPriceBigDec = osmomath.BigDecFromDec(tickToSqrtPrice(midtick))

		uppertick            = int64(74)
		upperSqrtPriceBigDec = osmomath.BigDecFromDec(tickToSqrtPrice(uppertick))
	)
	...
	for name, tc := range tests {
		...
		suite.Run(name, func() {
			...
			pool.CurrentSqrtPrice = osmomath.BigDecFromDec(currenTicktSqrtPrice)
