...
func (s *KeeperTestSuite) TestCalculateSpotPrice() {
	...
	s.Require().NoError(err)
	s.Require().Equal(spotPriceBaseUSDC, DefaultCurrSqrtPrice.PowerInteger(2))
	...
	s.Require().NoError(err)
	s.Require().Equal(spotPriceBaseETH, osmomath.OneDec().Quo(DefaultCurrSqrtPrice.PowerInteger(2)))
