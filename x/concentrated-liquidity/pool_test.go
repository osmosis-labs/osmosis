...
func (s *KeeperTestSuite) TestCalcOutAmtGivenIn() {
	ctx := s.Ctx
	pool, err := s.App.ConcentratedLiquidityKeeper.CreateNewConcentratedLiquidityPool(ctx, 1, testAddressOne, "eth", "usdc", sdk.MustNewDecFromStr("70.710678"), sdk.NewInt(85176))
...
func (s *KeeperTestSuite) TestCalcInAmtGivenOut() {
	ctx := s.Ctx
	pool, err := s.App.ConcentratedLiquidityKeeper.CreateNewConcentratedLiquidityPool(s.Ctx, 1, testAddressOne, "eth", "usdc", sdk.MustNewDecFromStr("70.710678"), sdk.NewInt(85176))
	s.Require().NoError(err)
	s.SetupPosition(pool.Id)
