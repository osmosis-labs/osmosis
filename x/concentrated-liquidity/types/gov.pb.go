...
// SetPoolHookContractProposal is a gov Content type for setting the hook
// contract for a set of actions on given pool. The proposal will fail if the
// given contract, pool, or action does not exist.
//
// For the sake of efficiency, one can create a single contract that supports
// the appropriate hook messages for multiple actions and set it for all of them
// at once. If one wants to roll out hook functionality such that each hook
// logic is in a separate contract, a separate gov proposal would need to be
// created for each of them.
type SetPoolHookContractProposal struct {
	Title                 string   `protobuf:"bytes,1,opt,name=title,proto3" json:"title,omitempty"`
	Description           string   `protobuf:"bytes,2,opt,name=description,proto3" json:"description,omitempty"`
	PoolId                uint64   `protobuf:"varint,3,opt,name=pool_id,json=poolId,proto3" json:"pool_id,omitempty"`
	HookActions           []string `protobuf:"bytes,4,rep,name=hook_actions,json=hookActions,proto3" json:"hook_actions,omitempty"`
	ContractAddressBech32 string   `protobuf:"bytes,5,opt,name=contract_address_bech32,json=contractAddressBech32,proto3" json:"contract_address_bech32,omitempty"`
}

func (m *SetPoolHookContractProposal) Reset()      { *m = SetPoolHookContractProposal{} }
func (*SetPoolHookContractProposal) ProtoMessage() {}
func (*SetPoolHookContractProposal) Descriptor() ([]byte, []int) {
	return fileDescriptor_a96adc35f4989ef7, []int{2}
}
func (m *SetPoolHookContractProposal) XXX_Unmarshal(b []byte) error {
	return m.Unmarshal(b)
}
func (m *SetPoolHookContractProposal) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	if deterministic {
		return xxx_messageInfo_SetPoolHookContractProposal.Marshal(b, m, deterministic)
	} else {
		b = b[:cap(b)]
		n, err := m.MarshalToSizedBuffer(b)
		if err != nil {
...
func (m *SetPoolHookContractProposal) XXX_Merge(src proto.Message) {
	xxx_messageInfo_SetPoolHookContractProposal.Merge(m, src)
}
func (m *SetPoolHookContractProposal) XXX_Size() int {
	return m.Size()
}
func (m *SetPoolHookContractProposal) XXX_DiscardUnknown() {
	xxx_messageInfo_SetPoolHookContractProposal.DiscardUnknown(m)
}

var xxx_messageInfo_SetPoolHookContractProposal proto.InternalMessageInfo
...
func (*PoolRecord) Descriptor() ([]byte, []int) {
	return fileDescriptor_a96adc35f4989ef7, []int{4}
}
...
func init() {
	...
	proto.RegisterType((*SetPoolHookContractProposal)(nil), "osmosis.concentratedliquidity.v1beta1.SetPoolHookContractProposal")
...
var fileDescriptor_a96adc35f4989ef7 = []byte{
	...
	,
}
...
func (this *SetPoolHookContractProposal) Equal(that interface{}) bool {
	if that == nil {
		return this == nil
	}

	that1, ok := that.(*SetPoolHookContractProposal)
	if !ok {
		that2, ok := that.(SetPoolHookContractProposal)
		if ok {
			that1 = &that2
		} else {
			return false
	...
	if len(this.HookActions) != len(that1.HookActions) {
		return false
	}
	for i := range this.HookActions {
		if this.HookActions[i] != that1.HookActions[i] {
			return false
		}
	}
	if this.ContractAddressBech32 != that1.ContractAddressBech32 {
		return false
	}
	return true
}
...
func (m *SetPoolHookContractProposal) Marshal() (dAtA []byte, err error) {
	size := m.Size()
	dAtA = make([]byte, size)
	n, err := m.MarshalToSizedBuffer(dAtA[:size])
	if err != nil {
...
func (m *SetPoolHookContractProposal) MarshalTo(dAtA []byte) (int, error) {
	size := m.Size()
	return m.MarshalToSizedBuffer(dAtA[:size])
}

func (m *SetPoolHookContractProposal) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	i := len(dAtA)
	_ = i
	var l int
	_ = l
	if len(m.ContractAddressBech32) > 0 {
		i -= len(m.ContractAddressBech32)
		copy(dAtA[i:], m.ContractAddressBech32)
		i = encodeVarintGov(dAtA, i, uint64(len(m.ContractAddressBech32)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.HookActions) > 0 {
		for iNdEx := len(m.HookActions) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.HookActions[iNdEx])
			copy(dAtA[i:], m.HookActions[iNdEx])
			i = encodeVarintGov(dAtA, i, uint64(len(m.HookActions[iNdEx])))
			i--
			dAtA[i] = 0x22
		}
	}
...
func (m *SetPoolHookContractProposal) Size() (n int) {
	if m == nil {
		return 0
	}
	var l int
	...
	if len(m.HookActions) > 0 {
		for _, s := range m.HookActions {
			l = len(s)
			n += 1 + l + sovGov(uint64(l))
		}
	}
	l = len(m.ContractAddressBech32)
	if l > 0 {
		n += 1 + l + sovGov(uint64(l))
	}
	return n
...
func (m *SetPoolHookContractProposal) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: SetPoolHookContractProposal: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SetPoolHookContractProposal: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field HookActions", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.HookActions = append(m.HookActions, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ContractAddressBech32", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ContractAddressBech32 = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skipGov(dAtA[iNdEx:])
