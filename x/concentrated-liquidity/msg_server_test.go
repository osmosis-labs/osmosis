...
func (suite *KeeperTestSuite) TestCollectIncentives_Events() {
	...
	for name, tc := range testcases {
		suite.Run(name, func() {
			...
			suite.Require().NoError(err)
			suite.FundAcc(pool.GetIncentivesAddress(), expectedIncentivesFromUptimeGrowth(uptimeHelper.hundredTokensMultiDenom, DefaultLiquidityAmt, positionAge, sdk.NewDec(int64(len(tc.positionIds)))))
