...
func (suite *StrategyTestSuite) TestComputeSwapStepOutGivenIn_OneForZero() {
	...
	tests := map[string]struct {
		...
		"3: 3% spread factor - reach target": {
			...
			amountOneInRemaining:     defaultAmountOne.Add(sdk.NewDec(100)).Quo(osmomath.OneDec().Sub(defaultSpreadReward)),
		...
		"4: 3% spread factor - do not reach target": {
			...
			sqrtPriceTarget:      sqrtPriceNext,
			liquidity:            defaultLiquidity,
			amountOneInRemaining: defaultAmountOne.Sub(sdk.NewDec(100)).QuoRoundUp(osmomath.OneDec().Sub(defaultSpreadReward)),
			spreadFactor:         defaultSpreadReward,
			...
			expectedSpreadRewardChargeTotal: defaultAmountOne.Sub(sdk.NewDec(100)).Quo(osmomath.OneDec().Sub(defaultSpreadReward)).Sub(defaultAmountOne.Sub(sdk.NewDec(100)).Ceil()),
		},
		"5: custom amounts at high price levels - reach target": {
			...
			liquidity:        math.GetLiquidityFromAmounts(osmomath.OneBigDec(), sqrt(100_000_000), sqrt(100_000_100), defaultAmountZero.TruncateInt(), defaultAmountOne.TruncateInt()),
