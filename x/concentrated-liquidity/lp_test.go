	...
	for name, tc := range tests {
		...
		s.Run(name, func() {
			...
			if !tc.preSetChargeSpreadRewards.IsZero() {
				err = clKeeper.ChargeSpreadRewards(s.Ctx, 1, tc.preSetChargeSpreadRewards)
				s.Require().NoError(err)
			}
...
func (s *KeeperTestSuite) TestWithdrawPosition() {
	...
	for name, tc := range tests {
		...
		s.Run(name, func() {
			...
			err = concentratedLiquidityKeeper.ChargeSpreadRewards(s.Ctx, pool.GetId(), globalSpreadRewardGrowth)
			s.Require().NoError(err)
