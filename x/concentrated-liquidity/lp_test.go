...
func (s *KeeperTestSuite) TestWithdrawPosition() {
	...
	for name, tc := range tests {
		...
		s.Run(name, func() {
			...
			poolBalanceBeforeWithdraw := s.App.BankKeeper.GetAllBalances(ctx, pool.GetAddress())
			incentivesBalanceBeforeWithdraw := s.App.BankKeeper.GetAllBalances(ctx, pool.GetIncentivesAddress())
			ownerBalancerBeforeWithdraw := s.App.BankKeeper.GetAllBalances(ctx, owner)
			...
			poolBalanceAfterWithdraw := s.App.BankKeeper.GetAllBalances(ctx, pool.GetAddress())
			incentivesBalanceAfterWithdraw := s.App.BankKeeper.GetAllBalances(ctx, pool.GetIncentivesAddress())
			ownerBalancerAfterWithdraw := s.App.BankKeeper.GetAllBalances(ctx, owner)
			...
			if expectedRemainingLiquidity.IsZero() {
				...
				position, err := concentratedLiquidityKeeper.GetPosition(ctx, config.positionId)
				s.Require().Error(err)
				...
				isPositionOwner, err := concentratedLiquidityKeeper.IsPositionOwner(ctx, owner, config.poolId, config.positionId)
				s.Require().NoError(err)
