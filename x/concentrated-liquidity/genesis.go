...
func (k Keeper) InitGenesis(ctx sdk.Context, genState genesis.GenesisState) {
	...
	for _, position := range genState.Positions {
		...
		k.setPosition(ctx, position.PoolId, position.Address.String(), position.LowerTick, position.UpperTick, position.JoinTime, position.FreezeDuration, position.Liquidity, position.PositionId)
	}
}
