...
func (k Keeper) computeOutAmtGivenIn(
	...
	for swapState.amountSpecifiedRemaining.GT(smallestDec) && !swapState.sqrtPrice.Equal(sqrtPriceLimit) {
		...
		ctx.Logger().Debug("reached sqrt price", computedSqrtPrice)
...
func (k Keeper) swapCrossTickLogic(ctx sdk.Context,
	...
	liquidityNet, err := k.crossTick(ctx, p.GetId(), nextInitializedTick, &nextInitializedTickInfo, sdk.NewDecCoinFromDec(tokenInDenom, swapState.spreadRewardGrowthGlobal), spreadRewardAccum.GetValue(), uptimeAccums)
	if err != nil {
		return swapState, err
	}
