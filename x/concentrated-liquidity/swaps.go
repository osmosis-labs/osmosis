...
func (k Keeper) computeOutAmtGivenIn(
	...
	for swapState.amountSpecifiedRemaining.GT(smallestDec) && !swapState.sqrtPrice.Equal(sqrtPriceLimit) {
		...
		emitAccumulatorUpdateTelemetry(ctx, types.SpreadFactorTruncationTelemetryName, spreadFactorsAccruedPerUnitOfLiquidity, spreadRewardCharge, poolId, swapState.liquidity, "is_out_given_in", "true")
...
func (k Keeper) computeInAmtGivenOut(
	...
	for swapState.amountSpecifiedRemaining.GT(smallestDec) && !swapState.sqrtPrice.Equal(sqrtPriceLimit) {
		...
		// Emit telemetry to detect spread reward truncation.
		emitAccumulatorUpdateTelemetry(ctx, types.SpreadFactorTruncationTelemetryName, spreadFactorsAccruedPerUnitOfLiquidity, spreadRewardChargeTotal, poolId, swapState.liquidity, "is_out_given_in", "false")
