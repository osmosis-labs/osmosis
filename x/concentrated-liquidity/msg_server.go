...
func (server msgServer) CreatePosition(goCtx context.Context, msg *types.MsgCreatePosition) (*types.MsgCreatePositionResponse, error) {
	...
	actualAmount0, actualAmount1, liquidityCeated, err := server.keeper.createPosition(ctx, msg.PoolId, sender, msg.TokenDesired0.Amount, msg.TokenDesired1.Amount, msg.TokenMinAmount0, msg.TokenMinAmount1, msg.LowerTick, msg.UpperTick)
	if err != nil {
		return nil, err
	}
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		...
		sdk.NewEvent(
			...
			sdk.NewAttribute(types.AttributeLiquidity, liquidityCeated.String()),
