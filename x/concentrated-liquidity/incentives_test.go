...
func expectedIncentives(denom string, rate sdk.Dec, timeElapsed time.Duration, qualifyingLiquidity sdk.Dec) sdk.DecCoin {
	...
	amount := rate.Mul(timeInSec).Quo(qualifyingLiquidity)
...
func (s *KeeperTestSuite) TestIncentiveRecordsSetAndGet() {
	...
	_, err = clKeeper.GetIncentiveRecord(s.Ctx, clPoolTwo.GetId(), incentiveRecordOne.IncentiveDenom, incentiveRecordOne.MinUptime)
	s.Require().Error(err)
