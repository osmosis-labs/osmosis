		...
		s.Run(tc.name, func() {
			...
			for _, positionDataEntry := range tc.expectedPositionData {
				...
				if positionDataEntry.LockId != 0 {
					actualLockId, err = clKeeper.GetPositionIdToLockId(ctx, positionDataEntry.Position.PositionId)
					s.Require().NoError(err)
				} else {
					_, err = clKeeper.GetPositionIdToLockId(ctx, positionDataEntry.Position.PositionId)
					s.Require().Error(err)
