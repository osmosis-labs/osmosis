...
func (s *KeeperTestSuite) crossTickAndChargeFee(poolId uint64, tickIndexToCross int64) {
	...
	feeAccum.AddToAccumulator(sdk.NewDecCoins(DefaultFeeAccumCoins[0]))

	// Cross the tick to update it.
	_, err = s.App.ConcentratedLiquidityKeeper.CrossTick(s.Ctx, poolId, tickIndexToCross, &nextTickInfo, feeAccum.GetValue(), uptimeAccums)
	s.Require().NoError(err)
