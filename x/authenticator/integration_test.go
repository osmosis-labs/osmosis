...
func (s *AuthenticatorSuite) TestCircuitBreaker() {
	...
	authenticatorParams.AuthenticatorActiveState = false
	...
	authenticatorParams.AuthenticatorActiveState = true
	s.app.AuthenticatorKeeper.SetParams(s.chainA.GetContext(), authenticatorParams)
...
func (s *AuthenticatorSuite) TestKeyRotation() {
	...
	err = s.app.AuthenticatorKeeper.RemoveAuthenticator(s.chainA.GetContext(), s.Account.GetAddress(), uint64(3))
