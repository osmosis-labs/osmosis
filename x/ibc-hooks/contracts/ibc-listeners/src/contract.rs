...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Subscribe {
            ...
            sequence,
            event,
        } => subscribe(deps, info.sender, channel, sequence, event),
    }
}
...
fn subscribe(
    ...
    LISTENERS.update(
        deps.storage,
        (&channel, sequence, &event.to_string()),
...
pub fn sudo(deps: DepsMut, _env: Env, msg: SudoMsg) -> Result<Response, ContractError> {
    ...
    match msg {
        SudoMsg::UnSubscribeAll { sequence, channel } => {
            LISTENERS.remove(
                deps.storage,
                (
                    &channel,
                    sequence,
                    &EventType::Acknowledgement {}.to_string(),
                ),
            );
            LISTENERS.remove(
                deps.storage,
                (&channel, sequence, &EventType::Timeout {}.to_string()),
            );
            Ok(Response::new())
        }
    }
    ...
    match msg {
        QueryMsg::Listeners {
            ...
            let maybe_list: Option<Vec<Addr>> =
                LISTENERS.may_load(deps.storage, (&channel, sequence, &event.to_string()))?;
