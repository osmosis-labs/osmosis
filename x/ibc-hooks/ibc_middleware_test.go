...
func (suite *HooksTestSuite) SetupCrosschainSwaps(withAckTracking bool) (sdk.AccAddress, sdk.AccAddress) {
	...
	crosschainAddr := suite.chainA.InstantiateContract(&suite.Suite,
		fmt.Sprintf(`{"swap_contract": "%s", "track_ibc_sends": %s}`, swaprouterAddr, trackAcks), 2)
...
func (suite *HooksTestSuite) TestCrosschainSwaps() {
	...
	msg := fmt.Sprintf(`{"osmosis_swap":{"input_coin":{"denom":"token0","amount":"1000"},"output_denom":"token1","slipage":{"max_slipage_percentage":"20"},"receiver":"%s","channel":"osmosis-test","failed_delivery":null}}`,
...
func (suite *HooksTestSuite) CrosschainSwapsViaIBCTest(withAckTracking bool) {
	...
	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"osmosis_swap":{"input_coin":{"denom":"token0","amount":"1000"},"output_denom":"token1","slipage":{"max_slipage_percentage":"20"},"receiver":"%s","channel":"osmosis-test","failed_delivery":null}}`,
		receiver,
	)
	// Generate full memo
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadAck() {
	...
	swapMsg := fmt.Sprintf(`{"osmosis_swap":{"input_coin":{"denom":"token0","amount":"1000"},"output_denom":"token1","slipage":{"max_slipage_percentage":"20"},"receiver":"%s","channel":"osmosis-bad-test","failed_delivery": {"recovery_addr": "%s"}}}`,
