...
func TestRandomizedGenState(t *testing.T) {
	...
	require.Equal(t, fmt.Sprintf("%s%s", expectedEpochProvisions.TruncateInt(), expectedDenom), mintGenesis.Minter.InflationProvisions(mintGenesis.Params).String())

	// Developer rewards epoch provisions
	expectedDevRewardsProvisions := epochProvisionsDec.Mul(devRewardsProportion)
	require.Equal(t, expectedDenom, mintGenesis.Params.MintDenom)
	require.Equal(t, fmt.Sprintf("%s%s", expectedDevRewardsProvisions.TruncateInt(), expectedDenom), mintGenesis.Minter.DeveloperVestingEpochProvisions(mintGenesis.Params).String())
}
