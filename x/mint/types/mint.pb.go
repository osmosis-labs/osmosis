...
type Minter struct {
	...
	// last_minted_total_amount represents the total amount of tokens minted since
	// the chain launched.
	// TODO: explain the difference from supply and supply with offset.
	LastMintedTotalAmount github_com_cosmos_cosmos_sdk_types.Dec `protobuf:"bytes,2,opt,name=last_minted_total_amount,json=lastMintedTotalAmount,proto3,customtype=github.com/cosmos/cosmos-sdk/types.Dec" json:"last_minted_total_amount" yaml:"last_minted_total_amount"`
}
...
>>>>>>> b41ef4821 (begin refactoring to handle truncations at the end of epoch)
}
...
func (m *Minter) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	_ = l
	{
		size := m.LastMintedTotalAmount.Size()
		i -= size
		if _, err := m.LastMintedTotalAmount.MarshalTo(dAtA[i:]); err != nil {
			return 0, err
		}
...
func (m *Minter) Size() (n int) {
	...
	l = m.LastMintedTotalAmount.Size()
	n += 1 + l + sovMint(uint64(l))
	return n
}
...
func (m *Minter) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field LastMintedTotalAmount", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if err := m.LastMintedTotalAmount.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
