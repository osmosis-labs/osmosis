...
func (suite *KeeperTestSuite) TestAfterEpochEnd() {
	...
	for name, tc := range testcases {
		suite.Run(name, func() {
			...
			mintKeeper.SetLastHalvenEpochNum(ctx, tc.preExistingEpochNum)
...
func (suite *KeeperTestSuite) TestAfterEpochEnd_FirstYearThirdening_RealParameters() {
	...
	mintKeeper.SetParams(ctx, mintParams)
	mintKeeper.SetLastHalvenEpochNum(ctx, 0)
	mintKeeper.SetMinter(ctx, types.Minter{
	...
	for i := int64(1); i <= defaultReductionPeriodInEpochs; i++ {
		...
		suite.Require().Equal(defaultMintingRewardsDistributionStartEpoch, mintKeeper.GetLastHalvenEpochNum(ctx))
	...
	suite.Require().Equal(defaultThirdeningEpochNum, mintKeeper.GetLastHalvenEpochNum(ctx))
