...
// at the end of each epoch, swap all non-MELODY fees into the desired denom and send either to fee collector or community pool.
...
func (k Keeper) swapNonNativeFeeToDenom(ctx sdk.Context, denomToSwapTo string, feeCollectorAddress sdk.AccAddress) {
	...
	for _, coin := range feeCollectorBalance {
		...
		// Since MELODY is one of the protorev denoms, many of the routes will exist in this store.
		...
		err = osmoutils.ApplyFuncIfNoErrorLogToDebug(ctx, func(cacheCtx sdk.Context) error {
			...
			// The only thing that could be done is a costly griefing attack to reduce the amount of melody given as tx fees.
