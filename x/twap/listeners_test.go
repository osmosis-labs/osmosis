...
func (s *TestSuite) TestCreateTwoAssetPoolFlow() {
	...
	expectedTwap, err := types.NewTwapRecord(s.App.GAMMKeeper, s.Ctx, poolId, denom1, denom0)
	s.Require().NoError(err)

	twap, err := s.twapkeeper.GetMostRecentRecordStoreRepresentation(s.Ctx, poolId, denom1, denom0)
	s.Require().NoError(err)
	s.Require().Equal(expectedTwap, twap)

	twap, err = s.twapkeeper.GetRecordAtOrBeforeTime(s.Ctx, poolId, s.Ctx.BlockTime(), denom1, denom0)
	s.Require().NoError(err)
	s.Require().Equal(expectedTwap, twap)
}
...
func (s *TestSuite) TestSwapAndEndBlockTriggeringSave() {
	...
	poolId := s.PrepareUni2PoolWithAssets(defaultUniV2Coins[0], defaultUniV2Coins[1])
	expectedHistoricalTwap, err := types.NewTwapRecord(s.App.GAMMKeeper, s.Ctx, poolId, denom1, denom0)
	s.Require().NoError(err)
	...
	expectedLatestTwapUpToAccum, err := types.NewTwapRecord(s.App.GAMMKeeper, s.Ctx, poolId, denom1, denom0)
	s.Require().NoError(err)
	...
	// check records
	historicalOldTwap, err := s.twapkeeper.GetRecordAtOrBeforeTime(s.Ctx, poolId, baseTime, denom1, denom0)
	s.Require().NoError(err)
	s.Require().Equal(expectedHistoricalTwap, historicalOldTwap)

	latestTwap, err := s.twapkeeper.GetMostRecentRecordStoreRepresentation(s.Ctx, poolId, denom1, denom0)
	s.Require().NoError(err)
	...
	latestTwap2, err := s.twapkeeper.GetRecordAtOrBeforeTime(s.Ctx, poolId, s.Ctx.BlockTime(), denom1, denom0)
	s.Require().NoError(err)
