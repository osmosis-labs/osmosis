...
func (s *TestSuite) TestGetRecordAtOrBeforeTime() {
	...
	baseRecord := withPrice0Set(newEmptyPriceRecord(1, baseTime, denom0, denom1))
	tMin1 := baseTime.Add(-time.Second)
	tMin1Record := withPrice0Set(newEmptyPriceRecord(1, tMin1, denom0, denom1))
	tPlus1 := baseTime.Add(time.Second)
	tPlus1Record := withPrice0Set(newEmptyPriceRecord(1, tPlus1, denom0, denom1))
