...
func (server msgServer) UpdateBlacklist(goCtx context.Context, msg *types.MsgUpdateBlacklist) (*types.MsgUpdateBlacklistResponse, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)

	authorityMetadata, err := server.Keeper.GetAuthorityMetadata(ctx, msg.GetDenom())
	if err != nil {
		return nil, err
	}
