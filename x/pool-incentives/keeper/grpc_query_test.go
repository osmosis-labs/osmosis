...
func (s *KeeperTestSuite) TestDistrInfo() {
	for _, tc := range []struct {
		...
		},
		{
			desc:                 "Happy case",
			poolCreated:          true,
			weights:              []sdk.Int{osmomath.NewInt(100), osmomath.NewInt(200), osmomath.NewInt(300)},
...
func (s *KeeperTestSuite) TestExternalIncentiveGauges() {
	...
	tests := map[string]struct {
		...
		"All gauges are internal (no external gauges)": {
			poolCreated:          true,
			internalGaugeWeights: []sdk.Int{osmomath.NewInt(100), osmomath.NewInt(200), osmomath.NewInt(300)},
