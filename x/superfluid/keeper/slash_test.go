...
func (s *KeeperTestSuite) TestPrepareConcentratedLockForSlash() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			feeAccumPreSlash, err := s.App.ConcentratedLiquidityKeeper.GetSpreadRewardsAccumulator(s.Ctx, clPoolId)
			s.Require().NoError(err)
			feePositionKey := cltypes.KeySpreadFactorPositionAccumulator(positionId)
			...
			if tc.expectedErr {
				...
				feeAccumPostSlash, err := s.App.ConcentratedLiquidityKeeper.GetSpreadRewardsAccumulator(s.Ctx, clPoolId)
				s.Require().NoError(err)
