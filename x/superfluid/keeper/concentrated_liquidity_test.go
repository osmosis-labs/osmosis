...
func (s *KeeperTestSuite) TestAddToConcentratedLiquiditySuperfluidPosition() {
	...
	for name, tc := range testCases {
		s.Run(name, func() {
			...
			osmoassert.Equal(s, errTolerance, expectedBondDenomSupplyDiff, diffInBondDenomSupply, fmt.Sprintf("expected (%s), actual (%s)", expectedBondDenomSupplyDiff, diffInBondDenomSupply))
			...
			osmoassert.Equal(s, errTolerance, tc.amount0Added, diffInPoolFundsToken0, fmt.Sprintf("expected (%s), actual (%s)", tc.amount0Added, diffInPoolFundsToken0))
			...
			osmoassert.Equal(s, errTolerance, tc.amount1Added, diffInPoolFundsToken1, fmt.Sprintf("expected (%s), actual (%s)", tc.amount1Added, diffInPoolFundsToken1))
			...
			osmoassert.Equal(s, errTolerance, expectedNewCoins[0].Amount, positionData.Amount0, fmt.Sprintf("expected (%s), actual (%s)", expectedNewCoins[0].Amount, positionData.Amount0))
			osmoassert.Equal(s, errTolerance, expectedNewCoins[1].Amount, positionData.Amount1, fmt.Sprintf("expected (%s), actual (%s)", expectedNewCoins[1].Amount, positionData.Amount1))
