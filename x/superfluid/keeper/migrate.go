...
func (k Keeper) UnlockAndMigrate(ctx sdk.Context, sender sdk.AccAddress, lockId uint64, sharesToMigrate sdk.Coin) (amount0, amount1 sdk.Int, liquidity sdk.Dec, joinTime time.Time, poolIdLeaving, poolIdEntering, newLockId uint64, freezeDuration time.Duration, err error) {
	...
	if sharesToMigrate.Amount.GT(gammSharesInLock.Amount) {
		return sdk.Int{}, sdk.Int{}, sdk.Dec{}, time.Time{}, 0, 0, 0, 0, fmt.Errorf("shares to migrate must be less than or equal to shares in lock")
	}
	...
	if len(exitCoins) != 2 {
		return sdk.Int{}, sdk.Int{}, sdk.Dec{}, time.Time{}, 0, 0, 0, 0, fmt.Errorf("Balancer pool must have exactly two tokens")
	}
	...
	return amount0, amount1, liquidity, joinTime, poolIdLeaving, poolIdEntering, newLockId, freezeDuration, nil
}
