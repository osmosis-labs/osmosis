...
type intermediaryDelegationTracker struct {
...
func (suite *KeeperTestSuite) SetupSuperfluidDelegate(delAddr sdk.AccAddress, valAddr sdk.ValAddress, denom string, amount int64) lockuptypes.PeriodLock {
	...
	suite.app.SuperfluidKeeper.SetOsmoEquivalentMultiplier(suite.ctx, 1, denom, sdk.NewDec(amount))
...
func (suite *KeeperTestSuite) TestRefreshIntermediaryDelegationAmounts1() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			// find original twaps
			originalTwaps := make(map[string]sdk.Dec)
			supplyShouldChange := false
			for _, denom := range tc.roundOneTwaps {
				twap, err := suite.app.SuperfluidKeeper.CalculateOsmoBackingPerShare(suite.ctx, denom.denom)
				suite.Require().NoError(err)
				originalTwaps[denom.denom] = twap

				if !twap.Equal(denom.price) {
					supplyShouldChange = true
				}
			}

			preRefreshIntermediaryDels := []stakingtypes.DelegationResponse{}
			for _, expAcc := range intermediaryAccs {
				valAddr, err := sdk.ValAddressFromBech32(expAcc.ValAddr)
				suite.Require().NoError(err)
				...
				delResponse, err := stakingkeeper.DelegationToDelegationResponse(suite.ctx, *suite.app.StakingKeeper, delegation)
				suite.Require().NoError(err)

				preRefreshIntermediaryDels = append(preRefreshIntermediaryDels, delResponse)
			}

			// create a map of new twap indexed by denom
			// set new pool prices before refresh
			twapByDenom := make(map[string]sdk.Dec)
			for _, twap := range tc.roundOneTwaps {
				twapByDenom[twap.denom] = twap.price

				poolId := gammtypes.MustGetPoolIdFromShareDenom(twap.denom)
				suite.SetTwapOfPool(poolId, twap.price)
			}
			...
			// run epochstartbeginblock in order to refresh intermediary account delegations
			suite.NotPanics(func() {
				suite.app.SuperfluidKeeper.AfterEpochStartBeginBlock(suite.ctx)
			})

			// ensure post-superfluid delegations osmo supplywithoffset is the same
			postsupplyWithOffset := suite.app.BankKeeper.GetSupplyWithOffset(suite.ctx, bondDenom)
			suite.Require().True(postsupplyWithOffset.IsEqual(presupplyWithOffset))

			// if twap has changed, then tokens should have been minted or burned, meaning pure supply should have changed
			postsupply := suite.app.BankKeeper.GetSupply(suite.ctx, bondDenom)
			if supplyShouldChange {
				suite.Require().False(postsupply.IsEqual(presupply), "presupply: %s   postsupply: %s", presupply, postsupply)
			} else {
				suite.Require().True(postsupply.IsEqual(presupply), "presupply: %s   postsupply: %s", presupply, postsupply)
			}

			for denom, expectedTwap := range twapByDenom {
				twap, err := suite.app.SuperfluidKeeper.CalculateOsmoBackingPerShare(suite.ctx, denom)
				suite.Require().NoError(err)

				suite.Require().True(twap.Equal(expectedTwap), "%s  %s", twap, expectedTwap)
			}

			for index, intermediaryAcc := range intermediaryAccs {
				twap := twapByDenom[intermediaryAcc.Denom]

				oldDelegation := preRefreshIntermediaryDels[index].Delegation
				oldDelegation.Shares = oldDelegation.Shares.Mul(twap).Quo(originalTwaps[intermediaryAcc.Denom])

				targetDelegationRes, err := stakingkeeper.DelegationToDelegationResponse(suite.ctx, *suite.app.StakingKeeper, oldDelegation)
				suite.Require().NoError(err)

				valAddr, _ := sdk.ValAddressFromBech32(intermediaryAcc.ValAddr)

				delegation, found := suite.app.StakingKeeper.GetDelegation(suite.ctx, intermediaryAcc.GetAccAddress(), valAddr)

				if targetDelegationRes.Balance.IsPositive() {
					suite.Require().True(found)
					delegationRes, err := stakingkeeper.DelegationToDelegationResponse(suite.ctx, *suite.app.StakingKeeper, delegation)
					suite.Require().NoError(err)
					suite.Require().Equal(delegation.Shares, targetDelegationRes.Delegation.Shares)
					suite.Require().True(delegationRes.Balance.Equal(targetDelegationRes.Balance), "%s  %s", delegationRes.Balance.String(), targetDelegationRes.Balance.String())
				} else {
			...
			// 	if targetDelegation.Balance.IsPositive() {
			// 		suite.Require().True(found)
			// 		suite.Require().Equal(delegation.Shares, targetDelegation.Delegation.Shares)
			// 	} else {
			// 		suite.Require().False(found)
			// 	}
			// }
