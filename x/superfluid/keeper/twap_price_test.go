...
func (s *KeeperTestSuite) TestGetSuperfluidOSMOTokens() {
	...
	osmoTokens, err := s.App.SuperfluidKeeper.GetSuperfluidOSMOTokensExcludeNonPool(s.Ctx, gammShareDenom, testAmount)
	s.Require().Error(err)
	...
	// Get superfluid OSMO tokens
	osmoTokens, err = s.App.SuperfluidKeeper.GetSuperfluidOSMOTokensExcludeNonPool(s.Ctx, clShareDenom, testAmount)
	s.Require().NoError(err)
