...
func (s *KeeperTestSuite) TestDistribute_ExternalIncentives_NoLock() {
	...
	type test struct {
		...
		expectErr                              bool
		expectedDistributions                  sdk.Coins
		expectedRemainingAmountIncentiveRecord []sdkmath.LegacyDec
	}
	...
	withGaugeCoins := func(tc test, gaugeCoins sdk.Coins) test {
		...
		tc.expectedRemainingAmountIncentiveRecord = make([]sdkmath.LegacyDec, len(gaugeCoins))
	...
	withNumEpochs := func(tc test, numEpochs uint64) test {
		...
		tempRemainingAmountIncentiveRecord := make([]sdkmath.LegacyDec, len(tc.expectedRemainingAmountIncentiveRecord))
