			...
			if tc.expectErr {
				...
				for _, gauge := range gauges {
					for _, coin := range gauge.Coins {
						...
						_, err = s.App.ConcentratedLiquidityKeeper.GetIncentiveRecord(s.Ctx, poolId, coin.Denom, currentEpoch.Duration)
						s.Require().Error(err)
					}
				}
			} else {
				...
				for _, gauge := range gauges {
					for _, coin := range gauge.Coins {
						...
						incentiveRecord, err := s.App.ConcentratedLiquidityKeeper.GetIncentiveRecord(s.Ctx, poolId, defaultRewardDenom, types.DefaultConcentratedUptime)
						s.Require().NoError(err)
		...
		s.Run(name, func() {
			...
			if tc.expectErr {
				...
				for i, coin := range tc.expectedDistributions {
					incentiveRecords, err := s.App.ConcentratedLiquidityKeeper.GetIncentiveRecord(s.Ctx, tc.poolId, coin.Denom, time.Nanosecond)
					s.Require().NoError(err)
