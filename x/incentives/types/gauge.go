...
// Creates a new gauge struct given the required gauge parameters.
...
// Returns true if the gauge's distribution start time is after the provided time.
func (gauge Gauge) IsUpcomingGauge(curTime time.Time) bool {
	return curTime.After(gauge.StartTime)
}

// Returns true if the gauge is in an active state during the provided time.
...
// Returns true if the gauge is in a finished state during the provided time.
