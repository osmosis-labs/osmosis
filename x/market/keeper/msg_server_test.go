...
func (s *KeeperTestSuite) TestMsgServer_SwapToNativeCoins() {
	...
	err = s.App.BankKeeper.SendCoinsFromModuleToModule(s.Ctx, FaucetAccountName, types.ModuleName, sdk.NewCoins(sdk.NewCoin(appparams.BaseCoinUnit, osmomath.NewInt(20000))))
	s.Require().NoError(err)
