...
func (s *KeeperTestSuite) TestComputeSwap() {
	...
	offerCoin := sdk.NewCoin(appparams.MicroSDRDenom, melodyPriceInSDR.QuoInt64(2).TruncateInt())
...
func (s *KeeperTestSuite) TestComputeInternalSwap() {
	...
	for i := 0; i < 100; i++ {
		offerCoin := sdk.NewDecCoin(appparams.MicroSDRDenom, melodyPriceInSDR.MulInt64(rand.Int63()+1).TruncateInt())
	...
	offerCoin := sdk.NewDecCoin(appparams.MicroSDRDenom, melodyPriceInSDR.QuoInt64(2).TruncateInt())
...
func (s *KeeperTestSuite) TestIlliquidTobinTaxListParams() {
	...
	s.App.OracleKeeper.SetMelodyExchangeRate(s.Ctx, appparams.MicroSDRDenom, melodyPriceInSDR)
	s.App.OracleKeeper.SetMelodyExchangeRate(s.Ctx, appparams.MicroMNTDenom, melodyPriceInMNT)
