...
func GetTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:                      fmt.Sprintf("%s transactions subcommands", streamswap.ModuleName),
...
func CreateSaleCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			fmt.Sprintf(`Create or Setup a streamswap sale.
Example:
$ %s tx streamswap create --sale-file="path/to/sale.json [flags]
...
func FinalizeSaleCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			fmt.Sprintf(`Finalize a streamswap sale.
Example:
$ %s tx streamswap finalize --sale-id=1 [flags]
`,
				version.AppName,
			),
		),
...
func SubscribeCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			fmt.Sprintf(`Subscribe or Join a streamswap sale.
Example:
$ %s tx streamswap subscribe --sale-id=1 --amount=10 (in the smallest denominator) [flags]
`,
				version.AppName,
			),
		),
...
func WithdrawCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			fmt.Sprintf(`Withdraw amount from a streamswap sale.
Example:
$ %s tx streamswap withdraw --sale-id=1 --amount=10 (in the smallest denominator) [flags]
`,
				version.AppName,
			),
		),
...
func ExitSaleCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short:   "Exit from a streamswap sale",
		Example: "exit --sale-id=1 [flags]",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Exit from a streamswap sale.
Example:
$ %s tx streamswap exit --sale-id=1 [flags]
`,
				version.AppName,
			),
		),
