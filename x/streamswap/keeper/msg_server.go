...
func (k Keeper) CreateSale(goCtx context.Context, msg *types.MsgCreateSale) (*types.MsgCreateSaleResponse, error) {
	...
	err = k.bank.SendCoinsFromAccountToModule(ctx, creator, streamswap.ModuleName, sdk.NewCoins(msg.TokenOut))
	if err != nil {
		return nil, err
	}
...
func (k Keeper) finalizeSale(ctx sdk.Context, msg *types.MsgFinalizeSale, store storetypes.KVStore) (sdk.Int, error) {
	...
	err = k.bank.SendCoinsFromModuleToAccount(ctx, streamswap.ModuleName, treasury, sdk.NewCoins(coin))
	if err != nil {
		return sdk.Int{}, err
	}
	income := p.Income
