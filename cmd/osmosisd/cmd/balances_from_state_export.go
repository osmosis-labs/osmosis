...
func underlyingCoins(originCoins sdk.Coins, pools map[string]gammtypes.CFMMI) sdk.Coins {
	balances := sdk.Coins{}
	convertAgain := false
...
func underlyingCoinsForSelectPools(
	...
	pools map[string]gammtypes.CFMMI,
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// collect gamm pools
			pools := make(map[string]gammtypes.CFMMI)
			for _, any := range gammGenesis.Pools {
				var pool gammtypes.CFMMI
