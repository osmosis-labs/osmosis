...
type SymphonyCustomClient struct {
...
func defaultClientConfig() *SymphonyCustomClient {
	return &SymphonyCustomClient{ChainID: chainID, KeyringBackend: keyringBackend, Output: output, Node: node, BroadcastMode: broadcastMode}
}

func (c *SymphonyCustomClient) SetChainID(chainID string) {
	c.ChainID = chainID
}

func (c *SymphonyCustomClient) SetKeyringBackend(keyringBackend string) {
	c.KeyringBackend = keyringBackend
}

func (c *SymphonyCustomClient) SetOutput(output string) {
	c.Output = output
}

func (c *SymphonyCustomClient) SetNode(node string) {
	c.Node = node
}

func (c *SymphonyCustomClient) SetBroadcastMode(broadcastMode string) {
...
###############################################################################
###                          Symphony Tx Configuration                       ###
###############################################################################
# Amount of gas per transaction
gas = "{{ .Gas }}"
# Price per unit of gas (ex: 0.005note)
...
func writeConfigToFile(configFilePath string, config *SymphonyCustomClient) error {
	var buffer bytes.Buffer
	defaultSymphonyCustomClient := defaultClientConfig()
	...
	if config != nil {
		...
		defaultValue := reflect.ValueOf(defaultSymphonyCustomClient).Elem()
	...
	if err := configTemplate.Execute(&buffer, defaultSymphonyCustomClient); err != nil {
		return err
	}
...
func getClientConfig(configPath string, v *viper.Viper) (*SymphonyCustomClient, error) {
