...
// func GetMelodySnapshot(inputFile string) (Snapshot, error) {
...
// func CosmosToMelodyAddress(cosmosAddr string) (string, error) {
// 	_, bz, err := bech32.DecodeAndConvert(cosmosAddr)
// 	if err != nil {
// 		return "", err
// 	}

// 	convertedAddr, err := bech32.ConvertAndEncode("melody", bz)
// 	if err != nil {
// 		panic(err)
// 	}
...
// Example:
// 	symphonyd import-genesis-accounts-from-snapshot networks/cosmoshub-3/snapshot.json networks/symphony-1/ions.json output_address.json
// `,
...
// 			// Read snapshot file
// 			snapshot, err := GetMelodySnapshot(args[0])
// 			if err != nil {
// 				return err
// 			}
...
// 				if !acc.MelodyBalance.Equal(osmomath.ZeroInt()) {
// 					melodyAddr, err := CosmosToMelodyAddress(acc.AtomAddress)
// 					if err != nil {
// 						return err
// 					}

// 					airdropAddrs[melodyAddr] = true
// 				}
// 			}

// 			for addr := range ionAmts {
// 				ionAddr, err := CosmosToMelodyAddress(addr)
// 				if err != nil {
// 					return err
// 				}
...
// Example:
// 	symphonyd import-genesis-accounts-from-snapshot ../snapshot.json ../ions.json
// 	- Check input genesis:
...
// 				address, err := CosmosToMelodyAddress(addr)
// 				if err != nil {
// 					return err
// 				}
...
// 			normalizationFactor := genesisParams.AirdropSupply.ToLegacyDec().QuoInt(snapshot.TotalMelodysAirdropAmount)
...
// 			for _, acc := range snapshot.Accounts {
// 				// convert cosmos address to melody address
// 				address, err := CosmosToMelodyAddress(acc.AtomAddress)
// 				if err != nil {
// 					return err
// 				}

// 				// skip accounts with 0 balance
// 				if !acc.MelodyBalanceBase.IsPositive() {
// 					continue
// 				}

// 				// get normalized melody balance for account
// 				normalizedMelodyBalance := acc.MelodyBalance.ToLegacyDec().Mul(normalizationFactor)

// 				// initial liquid amounts
// 				// We consistently round down to the nearest note
// 				liquidAmount := normalizedMelodyBalance.Mul(osmomath.MustNewDecFromStr("0.2")).TruncateInt() // 20% of airdrop amount
...
// 				// claimable balances
// 				claimableAmount := normalizedMelodyBalance.Mul(osmomath.MustNewDecFromStr("0.8")).TruncateInt()
...
// 			// The total airdrop melody is a smidge short (~1 melody) short of the stated 50M supply.
// 			// This is due to consistently rounding down.
// 			// We place this remaining 1 melody into the community pool at genesis
