		...
		s.Run(tc.name, func() {
			...
			if tc.haveTokensInAttribute {
				event.Attributes = append(event.Attributes, abcitypes.EventAttribute{
					...
					Value: strconv.Itoa(tokenInAmount) + defaultTokenInDenom,
					Index: false,
				})
			}
			...
			if tc.haveTokensInAttribute {
				...
				if tc.expectedAdjustedAmt {
					s.Require().NotEqual(tokenInAmount, adjustedTokensInAmount)
				} else {
					s.Require().Equal(tokenInAmount, adjustedTokensInAmount)
				}
			} else {
