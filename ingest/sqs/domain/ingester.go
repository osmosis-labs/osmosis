...
type PoolsTransformer interface {
	...
	Transform(ctx sdk.Context, blockPools commondomain.BlockPools) ([]types.PoolI, types.TakerFeeMap, error)
}
...
type SQSGRPClient interface {
	...
	PushData(ctx context.Context, height uint64, pools []types.PoolI, takerFeesMap types.TakerFeeMap) error
}
