...
e2e-docker-build-e2e-init-chain:
	@DOCKER_BUILDKIT=1 docker build --build-arg -t osmolabs/osmosis-e2e-init-chain:debug --build-arg E2E_SCRIPT_NAME=chain --platform=linux/x86_64 -f tests/e2e/initialization/init.Dockerfile .

e2e-docker-build-e2e-init-node:
	@DOCKER_BUILDKIT=1 docker build --build-arg -t osmosis-e2e-init-node:debug --build-arg E2E_SCRIPT_NAME=node --platform=linux/x86_64 -f tests/e2e/initialization/init.Dockerfile .
