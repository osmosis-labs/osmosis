...
e2e-docker-build-debug:
	@DOCKER_BUILDKIT=1 docker build --build-arg BUILD_TAGS="netgo,muslc" -t osmosis:${COMMIT} --build-arg BASE_IMG_TAG=debug --build-arg RUNNER_IMAGE=$(RUNNER_BASE_IMAGE_ALPINE) -f Dockerfile .
	@DOCKER_BUILDKIT=1 docker tag osmosis:${COMMIT} osmosis:debug

e2e-docker-build-e2e-init-chain:
	@DOCKER_BUILDKIT=1 docker build --build-arg BUILD_TAGS="netgo,muslc" -t osmolabs/osmosis-e2e-init-chain:debug --build-arg E2E_SCRIPT_NAME=chain --platform=linux/x86_64 -f tests/e2e/initialization/init.Dockerfile .

e2e-docker-build-e2e-init-node:
	@DOCKER_BUILDKIT=1 docker build --build-arg BUILD_TAGS="netgo,muslc" -t osmosis-e2e-init-node:debug --build-arg E2E_SCRIPT_NAME=node --platform=linux/x86_64 -f tests/e2e/initialization/init.Dockerfile .
