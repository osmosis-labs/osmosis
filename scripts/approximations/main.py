...
def approximated_fn(x): return sp.Pow(2, x)
...
def plot_error_range(x_coordinates, y_approximation, y_actual, is_absolute: bool):
    ...
    if is_absolute:
        ...
        error_deltas = approximations.compute_absolute_error_range(
            y_approximation, y_actual)
    else:
        error_kind_str = "Relative"
        error_deltas = approximations.compute_relative_error_range(
            y_approximation, y_actual)
    ...
    plt.grid(True)
    plt.title(
        f"Chebyshev Rational e^x {error_kind_str} Errors on [{x_start}, {x_end}]. {num_parameters} params, {num_points_plot} points")
    plt.show()
...
def main():
    ...
    if shouldComputeErrorDelta or shouldPlotApproximations or shouldPlotErrorRange:
        ...
        y_eqispaced_poly, y_chebyshev_poly, y_chebyshev_rational, y_actual = approximations.approx_and_eval_all(
            approximated_fn, num_parameters, x_coordinates)
        ...
        if shouldComputeErrorDelta:
            ...
            max_error_equispaced_poly = approximations.compute_max_error(
                y_eqispaced_poly, y_actual)
            print(
                F"Equispaced Poly: {max_error_equispaced_poly.evalf(chop=1e-100)}")

            # Chebyshev Polynomial Approximation
            max_error_chebyshev_poly = approximations.compute_max_error(
                y_chebyshev_poly, y_actual)
            print(
                F"Chebyshev Poly: {max_error_chebyshev_poly.evalf(chop=1e-100)}")

            # Chebyshev Rational Approximation
            max_error_chebyshev_rational = approximations.compute_max_error(
                y_chebyshev_rational, y_actual)
            print(
                F"Chebyshev Rational: {max_error_chebyshev_rational.evalf(chop=1e-100)}")
        ...
        if shouldPlotApproximations:
            ...
            # Chebyshev Rational Approximation
            plt.plot(x_coordinates, y_chebyshev_rational,
                     label="Chebyshev Rational")
            ...
            plt.grid(True)
            plt.title(
                f"Appproximation of e^x on [{x_start}, {x_end}] with {num_parameters} parameters")
            plt.show()
    ...
    if shouldPlotMaxError:
        ...
        for num_parameters_current in range(1, num_parameters_errors + 1):
            ...
            y_eqispaced_poly, y_chebyshev_poly, y_chebyshev_rational, y_actual = approximations.approx_and_eval_all(
                approximated_fn, num_parameters_current, x_coordinates)

            deltas_eqispaced_poly.append(
                approximations.compute_max_error(y_eqispaced_poly, y_actual))
            deltas_chebyshev_poly.append(
                approximations.compute_max_error(y_chebyshev_poly, y_actual))
            deltas_chebyshev_rational.append(
                approximations.compute_max_error(y_chebyshev_rational, y_actual))
        ...
        # Chebyshev Rational Approximation
        plt.semilogy(x_axis, deltas_chebyshev_rational,
                     label="Chebyshev Rational")
        ...
        plt.ylabel(
            F"-log_10{{ max | f'(x) - f(x) | }} where x is in [{x_start}, {x_end}]")
        plt.show()
...
def exponent_approximation_choice():
    # Equispaced x coordinates to be used for plotting every approximation.
    x_coordinates = approximations.linspace(x_start, x_end, num_points_plot)
    x_coordinates = [sp.N(sp.Float(coef, osmomath_precision),
                          n=osmomath_precision) for coef in x_coordinates]
    ...
    coef_numerator = [
        sp.N(sp.Float("1.000000000000000000000044212244679434",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.352032455817400196452603772766844426",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.056507868883666405413116800969512484",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.005343900728213034434757419480319916",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.000317708814342353603087543715930732",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.000011429747507407623028722262874632",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.000000198381965651614980168744540366",
             osmomath_precision), n=osmomath_precision),
    ]

    coef_denominator = [
        sp.N(sp.Float("1.0000000000000000000000000000000000000",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("-0.341114724742545112949699755780593311",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.052724071627342653404436933178482287",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("-0.004760950735524957576233524801866342",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.000267168475410566529819971616894193",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("-0.000008923715368802211181557353097439",
             osmomath_precision), n=osmomath_precision),
        sp.N(sp.Float("0.000000140277233177373698516010555916",
             osmomath_precision), n=osmomath_precision),
    ]
    ...
    y_chebyshev_rational = rational.evaluate(
        x_coordinates, coef_numerator, coef_denominator)
